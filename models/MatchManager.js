...
const matchAlg = () => {
  ...
  if (playerCount >= matchType) {
    ...
    if (loopCounter == matchType) {
      console.log(
        `We have found all the potential player for a ${matchType}P match`
      );
      ...
      for (const { socket, matchIndex } of sockets) {
        const matchD = { ...newMatch, matchIndex: matchIndex };
        // console.log(JSON.stringify(matchD));
        socket.emit("matchFound", matchD);
      }
    } else {
      playerList[matchedPlayers.map(item => item.id)].reset(2);

      for ({ id } of matchedPlayers) {
